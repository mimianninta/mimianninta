...
1. git clone
2. goto fish_detection & run commands for reqired installation
...
4. goto fish_detection/models/research and run
...
Goto fish_detection & Run

```
!python create_tfrecords.py --output_path data/fish_train.record --csv_input data/fish_train.csv --image_path data/images/train/
```

```
!python create_tfrecords.py --output_path data/fish_val.record --csv_input data/fish_val.csv --image_path data/images/valid/
```
...
From `fish_detection/data/fish_label_map.pbtxt`, you can see sample label maps.
...
Pick the model's config from `/fish_detection/models/research/object_detection/samples/configs`. Duplicate it somewhere.

**Note** : you must pick the config with the **same** name as your model.

Or Modify the config from `fish_detection/faster_rcnn_inception_v2_coco.config`
...
From `fish_detection/models/research/`, run
```
